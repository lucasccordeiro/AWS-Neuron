# Positive-control variant of attn_fwd_v1_asym: the first matmul's
# transpose flags are flipped (`True, False` -> `False, True`). With the
# upstream's symmetric 128 x 128 shapes this mutation is invisible — both
# operands have d0 == d1 == 128, so the contraction axis is 128 regardless
# of which transpose flag is set, and `k_x == k_y` trivially holds. With
# the asymmetric driver (d_head = 128, seqlen_q = 64, seqlen_k = 32) the
# flip changes the contraction axis to a non-matching length and
# nl_matmul's `assert k_x == k_y` fires.
#
# This is exactly the *toy-shape blind spot* recorded in ROADMAP.md
# §70-79: a mutation that the contract suite cannot discriminate when
# every input is square. The asymmetric driver closes the discrimination
# gap and catches the bug.
...
def attn_fwd_v1_asym(q: Tile, k: Tile, v: Tile) -> Tile:
    ...
    qk_psum: Tile = nl_matmul(q_sbuf, k_sbuf, False, True)  # BUG: was (True, False)
