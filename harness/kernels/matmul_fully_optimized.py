...
#
# nki_matmul_fully_optimized_ — blocks all three of M, N, K and keeps a
# result accumulator in SBUF across the K-block loop. Upstream uses
# three-level nested Python lists `result_tmps[m][bm][bn]`,
# `lhsT_tiles[bk]`, `rhs_tiles[bk]`. We model each multi-level list as
# a Tile3D with the outer indices flattened into one slab axis, and
# slice each slab with `t[slot, :, c0:c1]` to extract per-(bm/bn) views.
