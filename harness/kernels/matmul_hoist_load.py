...
#
# nki_matmul_hoist_load_ — same as tiled, but hoists the per-(k) lhsT
# loads out of the n loop and the per-(k) rhs loads out of the k loop.
# The upstream `lhsT_tiles = []; lhsT_tiles.append(...)` Python lists
# are modelled as Tile3D slabs (slab_set / slab_get), since ESBMC's
