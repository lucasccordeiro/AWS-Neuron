...
#
# This variant injects a wrong-stride bug on the destination column index to
# demonstrate the counterexample ESBMC produces. The upstream file is
# correct as published.
#
# NKI transpose2d kernel — ESBMC-Python PoC, buggy variant.
#
...
def tensor_transpose2D_kernel(in_tensor: Tile, sz_f1: int, sz_f2: int) -> Tile:
    ...
    while i_f1 < sz_f1:
        i_f2: int = 0
        while i_f2 < sz_f2:
            # BUG: source uses sz_f2 instead of sz_f1 as the destination
            # stride. For (i_f1=F1-1, i_f2=F2-1) this lands at i_f2*sz_f2 +
            # i_f1 = (F2-1)*F2 + (F1-1), which exceeds sz_f1 * sz_f2 = F1*F2
            # whenever F2 > F1.
