...
#
# Positive control for the matmul port: same kernel as matmul_contributed.py
# with one synthetic off-by-one on the inner-loop column-end so we have
# evidence ESBMC would catch a real bug in this kernel if one existed. The
# upstream file is correct as published; do not interpret the failing run
# as a defect in AWS code.
            ...
            while k2 < NUM_BLOCK_K:
                ...
                while m1 < TILES_IN_BLOCK_M:
                    ...
                    while n1 < TILES_IN_BLOCK_N:
                        ...
                        # Produces a slab strip of width TILE_M+1 = 129, which
                        # exceeds GEMM_STATIONARY_FMAX = 128 on the matmul
                        # stationary input.
