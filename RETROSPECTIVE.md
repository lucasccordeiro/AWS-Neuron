...
| [#4545](https://github.com/esbmc/esbmc/issues/4545) | **PARTIALLY RESOLVED** | `__getitem__` on a parameter whose class is imported from another module crashes at BMC symex (`symbolic_type_excp`) | 2-file repro (main + stubs) now verifies SUCCESSFUL after the PR #4549 + #4551 fixes landed; the **3-file transitive-import case** (main → kernels → stubs, where the kernel uses natural-form `__getitem__` on a parameter) still crashes. That's the actual PoC blocker, since every harness imports the kernel which imports stubs. Keeps `slice2d` / `slice_cols` in place across all 62 two-axis call sites |
...
| [#4552](https://github.com/esbmc/esbmc/issues/4552) | OPEN | two cross-module classes with `__getitem__` at different subscript arities crash with `type mismatch: got pointer, expected struct` when both are used in the same TU | blocks the higher-arity slice retirement (`slice_3d_at`, `slice_4d_drop_d0_d1`, `slab_cols_get`/`_set`) — adding `__getitem__` to Tile + Tile3D + Tile4D in the same `stubs.py` would trip this once cross-module imports work cleanly |
...
| `a[i:j, k:l]` | `slice2d(a, i, j, k, l)` free-function call | (active) | `__getitem__` assertion crash from #4514 retired by [PR #4522](https://github.com/esbmc/esbmc/pull/4522); conversion-time slice handling from #4523 retired by [PR #4528](https://github.com/esbmc/esbmc/pull/4528); single-slice `a[i:j]` from #4537 retired by [PR #4538](https://github.com/esbmc/esbmc/pull/4538); tuple-of-slices `a[i:j, k:l]` for local instances from #4539 retired by [PR #4540](https://github.com/esbmc/esbmc/pull/4540); parameter-instance receiver from #4541 retired by [PR #4544](https://github.com/esbmc/esbmc/pull/4544) for the single-file case; heterogeneous-tuple key threading from #4542 retired by [PR #4549](https://github.com/esbmc/esbmc/pull/4549); bare-`:` modelling from #4543 retired by [PR #4551](https://github.com/esbmc/esbmc/pull/4551); the 2-file cross-module case from #4545 now also verifies as an incidental effect of #4549 + #4551. Residual blocker is the **3-file transitive-import case** of #4545: every PoC harness imports its kernel which imports `stubs.py`, and that import chain still crashes BMC symex with `symbolic_type_excp` when the kernel uses natural-form `__getitem__` on its imported-type parameter. A second blocker for the higher-arity sites is [#4552](https://github.com/esbmc/esbmc/issues/4552) (two cross-module classes with `__getitem__` at different arities). |
