...
| [#4541](https://github.com/esbmc/esbmc/issues/4541) | OPEN | `__getitem__` on a function-parameter instance crashes (`make_member` assertion when slice indices are constants; `symbolic_type_excp` when parametric) | every PoC kernel takes Tiles as parameters and slices them, so this keeps `slice2d` / `slice_cols` in place across all 62 two-axis call sites |
...
| `a[i:j, k:l]` | `slice2d(a, i, j, k, l)` free-function call | (active) | `__getitem__` assertion crash from #4514 retired by [PR #4522](https://github.com/esbmc/esbmc/pull/4522); conversion-time slice handling from #4523 retired by [PR #4528](https://github.com/esbmc/esbmc/pull/4528); single-slice `a[i:j]` from #4537 retired by [PR #4538](https://github.com/esbmc/esbmc/pull/4538); tuple-of-slices `a[i:j, k:l]` for instances constructed in the calling scope from #4539 retired by [PR #4540](https://github.com/esbmc/esbmc/pull/4540). Residual blocker is **parameter-instance receivers** — every PoC kernel takes Tiles as parameters and slices them, and that pattern still crashes at `make_member` (constant slice indices) or `symbolic_type_excp` (parametric), filed as follow-on [#4541](https://github.com/esbmc/esbmc/issues/4541) |
