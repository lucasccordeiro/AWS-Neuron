...
- **19 NKI kernel functions ported** (across 11 upstream source files —
  ...
  shape-skeleton port only), 55 phase-1 build targets (concrete +
  positive-control + 13 symbolic-shape variants + 1 historical-bug
  reproduction + 1 shape-skeleton + 1 partial-pipeline port
  (`pipelined_attention_load_q`) + 2 boundary-input regression
  targets) + 35 phase-2 runs (every concrete- and symbolic-shape good
