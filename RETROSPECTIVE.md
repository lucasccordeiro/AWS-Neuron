...
| [#4554](https://github.com/esbmc/esbmc/issues/4554) | OPEN | `__getitem__` on imported-class parameter still crashes in 3-file transitive-import form (main → kernel module → stubs) | this is the exact PoC layout; every harness does `from stubs import *; from kernels.<name> import <kernel>` and the kernel slices its imported-type parameter. Closing it retires the 62 two-axis `slice2d` / `slice_cols` call sites |
