...
| [#4558](https://github.com/esbmc/esbmc/issues/4558) | OPEN | `__getitem__` on imported Tile3D/Tile4D crashes when the first scalar element of a tuple-key of arity ≥ 3 is a Python variable (loop index, parameter, local) — `symbolic_type_excp` for Tile3D, `dereference failure: Incorrect alignment` for Tile4D | every higher-arity site is `t[i, ...]` with `i` a loop variable; keeps the ~86 sites (`slice_3d_at`, `slice_4d_drop_d0_d1`, `slab_get`/`set`, `slab_cols_get`/`set`) in place |
